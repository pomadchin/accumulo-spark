#!/bin/bash
docker run -d -t --dns 127.0.0.1 -e NODE_TYPE=s -e ZOOKEEPER_ID=2 --name slave1 -h slave1.owm daunnc/hadoop-dn:0.2
FIRST_IP=$(docker inspect --format="{{.NetworkSettings.IPAddress}}" slave1)
docker run -d -t --dns 127.0.0.1 -e NODE_TYPE=s -e ZOOKEEPER_ID=3 -e JOIN_IP=$FIRST_IP --name slave2 -h slave2.owm daunnc/hadoop-dn:0.2
