...
docker run -d -t --dns 127.0.0.1 \ 
           -e NODE_TYPE=s \ 
           -e ZOOKEEPER_ID=2 --name slave1 -h slave1.owm daunnc/hadoop-dn:0.2
...
docker run -d -t --dns 127.0.0.1 \
           -e NODE_TYPE=s \
           -e ZOOKEEPER_ID=3 \
           -e JOIN_IP=$FIRST_IP --name slave2 -h slave2.owm daunnc/hadoop-dn:0.2
...
docker run -d -t --dns 127.0.0.1 \
           ...
           -p 1808:1808 --name master1 -h master1.owm daunnc/hadoop-nn-dn:0.2
